...
import com.linecorp.lich.sample.databinding.NavGraphFirstFragmentBinding
...
class FirstFragment : Fragment(R.layout.nav_graph_first_fragment) {

    private val navHostViewModel by activityViewModel(NavHostViewModel)

    private val navGraphViewModel by navGraphViewModel(NavDestinationViewModel, R.id.main_nav_graph)

    private val fragmentViewModel by viewModel(NavDestinationViewModel)

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        val binding = NavGraphFirstFragmentBinding.bind(view)
        ...
        binding.fooChildButton.setOnClickListener(
            Navigation.createNavigateOnClickListener(
                R.id.action_to_child_graph,
                NavDestinationViewModelArgs("ARG_FOO").toBundle()
            )
        )
        binding.barChildButton.setOnClickListener(
            Navigation.createNavigateOnClickListener(
                R.id.action_to_child_graph,
                NavDestinationViewModelArgs("ARG_BAR").toBundle()
            )
        )
    }
}
